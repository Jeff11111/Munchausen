...
SUBSYSTEM_DEF(ticker)
	...
	var/round_start_sound = 'modular_skyrat/sound/dementia/roundstart.ogg'	//music/jingle played when the round starts
...
/datum/controller/subsystem/ticker/proc/transfer_characters()
	...
	for(var/mob/dead/new_player/player in GLOB.mob_list)
		...
		if(living)
			...
			if(living.client)
				if(living.client.prefs?.auto_ooc & CHAT_OOC)
...
/proc/send_gamemode_vote()
	if(SSticker.current_state == GAME_STATE_PREGAME)
		...
		if(dynamic)
			SSvote.initiate_vote("dynamic", "server", display = NONE, votesystem = SCORE_VOTING, forced = TRUE,vote_time = 2 MINUTES)
		else
			SSvote.initiate_vote("roundtype", "server", display = NONE, votesystem = PLURALITY_VOTING, forced=TRUE, \
			vote_time = (CONFIG_GET(flag/modetier_voting) ? 1 MINUTES : 2 MINUTES))
...
/datum/controller/subsystem/ticker/proc/send_news_report()
	...
	switch(news_report)
		...
		if(CLOCK_SUMMON)
			news_message = "The garbled messages about hailing a mouse and strange energy readings from [station_name()] have been discovered to be an ill-advised, if thorough, prank by a jester."
		if(CLOCK_SILICONS)
