...
/proc/printobjectives(list/objectives)
	...
	for(var/datum/objective/objective in objectives)
		if(objective.completable)
			...
			if(completion >= 1)
				objective_parts += "<B>[objective.flavor] #[count]</B>: [objective.explanation_text] <span class='greentext'><B>Success!</B></span>"
			else if(completion <= 0)
				objective_parts += "<B>[objective.flavor] #[count]</B>: [objective.explanation_text] <span class='redtext'>Fail.</span>"
			else
				objective_parts += "<B>[objective.flavor] #[count]</B>: [objective.explanation_text] <span class='yellowtext'>[completion*100]%</span>"
		else
			objective_parts += "<B>[objective.flavor] #[count]</B>: [objective.explanation_text]"
		count++
