...
/datum/element/flavor_text/proc/set_flavor(mob/user)
	...
	var/new_text = input(user, "Set the [lower_name] displayed on 'examine'. [addendum]", flavor_name, texts_by_atom[usr]) as message|null
	if(!isnull(new_text) && (user in texts_by_atom))
		texts_by_atom[user] = strip_html_simple(new_text, MAX_FLAVOR_LEN, TRUE)
