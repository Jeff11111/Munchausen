/proc/create_message(type, target_key, admin_ckey, text, timestamp, server, secret, logged = 1, browse, expiry, note_severity)
	...
	if(logged)
		...
		admin_ticket_log(target_ckey, "<font color='blue'>[header]</font>" , private = TRUE)
		admin_ticket_log(target_ckey, text, private = TRUE)
		if(browse)
...
/proc/browse_messages(type, target_ckey, index, linkless = FALSE, filter, agegate = FALSE)
	...
	if(target_ckey)
		...
		while(query_get_messages.NextRow())
			...
			data += "<b>[timestamp] | [server] | [admin_key] | [get_exp_format(text2num(playtime))][secret ? " | <i>- Secret</i>" : ""]"
			if(expire_timestamp)
	...
	while(query_get_message_output.NextRow())
		...
		switch(type)
			...
			if("watchlist entry")
				...
				add_to_tracked_watchlist(target_ckey)
			if("memo")
