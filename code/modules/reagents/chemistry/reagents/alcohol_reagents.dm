	...
	if(prob(5) && iscarbon(M))
		...
		if(HAS_TRAIT(M, TRAIT_BLIND))
			if(LE || RE)
				...
				M.death_scream()
				M.apply_damage(15, BURN, BODY_ZONE_HEAD)
		else
...
/datum/reagent/consumable/ethanol/screwdrivercocktail/on_mob_life(mob/living/carbon/M)
	if(M.mind && (M.mind.assigned_role in list("Station Engineer", "Atmospheric Technician", "Senior Engineer"))) //Engineers lose radiation poisoning at a massive rate.
