...
/obj/machinery/rnd/production/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		. += "<span class='notice'>The status display reads: Storing up to <b>[materials.local_size]</b> material units locally.<br>Material usage cost at <b>[CEILING(print_cost_coeff*100,5)]%</b>.</span>"
...
/obj/machinery/rnd/production/proc/check_mat(datum/design/being_built, var/mat)	// now returns how many times the item can be built with the material
	...
	return round(A / max(1, MAT_COST_WITH_COEFF(all_materials[mat],ef)))
...
/obj/machinery/rnd/production/proc/user_try_print_id(id, amount)
	...
	for(var/MAT in D.materials)
		efficient_mats[MAT] = MAT_COST_WITH_COEFF(D.materials[MAT],coeff)
	...
	for(var/R in D.reagents_list)
		if(!reagents.has_reagent(R, MAT_COST_WITH_COEFF(D.reagents_list[R],coeff) * amount))
	...
	for(var/R in D.reagents_list)
		reagents.remove_reagent(R, MAT_COST_WITH_COEFF(D.reagents_list[R],coeff) * amount)
	busy = TRUE
...
/obj/machinery/rnd/production/proc/design_menu_entry(datum/design/D, coeff)
	...
	for(var/M in all_materials)
		...
		temp_material += " | "
		if (t < 1)
			temp_material += "<span class='bad'>[MAT_COST_WITH_COEFF(all_materials[M],coeff)] [CallMaterialName(M)]</span>"
		else
			temp_material += " [MAT_COST_WITH_COEFF(all_materials[M],coeff)] [CallMaterialName(M)]"
		c = min(c,t)
