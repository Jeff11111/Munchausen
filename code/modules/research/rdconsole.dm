	...
	for(var/v in stored_research.researched_designs)
		...
		for(var/M in all_materials)
			...
			if (!linked_imprinter.check_mat(D, M))
				...
				temp_materials += " <span class='bad'>[MAT_COST_WITH_COEFF(all_materials[M],coeff)] [CallMaterialName(M)]</span>"
			else
				temp_materials += " [MAT_COST_WITH_COEFF(all_materials[M],coeff)] [CallMaterialName(M)]"
		if (check_materials)
