...
/mob/living/simple_animal/bot/secbot/UnarmedAttack(atom/A, attackchain_flags)
	...
	if(iscarbon(A))
		...
		if(CHECK_MOBILITY(C, MOBILITY_MOVE|MOBILITY_USE|MOBILITY_STAND) || arrest_type)
			stun_attack(A)
...
/mob/living/simple_animal/bot/secbot/handle_automated_action()
	...
	switch(mode)
		...
		if(BOT_PREP_ARREST)		// preparing to arrest target
			...
			if( !Adjacent(target) || !isturf(target.loc) || target.getStaminaLoss() <= 120 || !(target.combat_flags & COMBAT_FLAG_HARD_STAMCRIT))
				back_to_hunt()
				return
