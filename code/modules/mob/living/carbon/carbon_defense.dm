...
/mob/living/carbon/attacked_by(obj/item/I, mob/living/user, attackchain_flags = NONE, damage_multiplier = 1)
	...
	var/impacting_zone = (user == src) ? check_zone(user.zone_selected) : ran_zone(check_zone(user.zone_selected))
	...
	if(I.force)
		apply_damage(totitemdamage, I.damtype, affecting, wound_bonus = I.wound_bonus, bare_wound_bonus = I.bare_wound_bonus, sharpness = I.get_sharpness())
		...
		if(I.damtype == BRUTE && affecting.is_organic_limb())
			...
			if(prob(basebloodychance))
				...
				if(istype(targ) && dist > 0 && ((mob_biotypes & MOB_ORGANIC) || (mob_biotypes & MOB_HUMANOID)) && needs_heart() && !is_asystole())
		...
		for(var/obj/item/I in LB.embedded_objects)
			if(!embeds)
				...
				visible_message("<span class='notice'><b>[src]</b> examines [p_themselves()].</span>", \
					"<span class='notice'>You check yourself for shrapnel.</span>")
			if(I.isEmbedHarmless())
				to_chat(src, "\t <a href='?src=[REF(src)];embedded_object=[REF(I)];embedded_limb=[REF(LB)]' class='warning'>There is \a [I] stuck to my [LB.name]!</a>")
			else
				to_chat(src, "\t <a href='?src=[REF(src)];embedded_object=[REF(I)];embedded_limb=[REF(LB)]' class='warning'>There is \a [I] embedded in my [LB.name]!</a>")
...
/mob/living/carbon/proc/clothingonpart(obj/item/bodypart/def_zone)
	var/list/covering_part = list()
	var/list/body_parts = list(head, wear_mask, wear_neck, back)
