...
/mob/living/carbon/human/proc/do_climax(datum/reagents/R, atom/target, obj/item/organ/genital/G, spill = TRUE)
	...
	message_admins("[ADMIN_LOOKUPFLW(src)] came[target ?  " on [ismob(target) ? "[ADMIN_LOOKUPFLW(target)]" : "[target.name]"]" : " alone"][G ? " with [p_their()] [G.name]" : ""][spill ? ", spilling [p_their()] juices" : ""]. ")
...
/mob/living/carbon/human/proc/mob_climax(forced_climax=FALSE, mob/living/forced_partner = null, forced_spillage = TRUE) //Forced is instead of the other proc, makes you cum if you have the tools for it, ignoring restraints
	...
	if(forced_climax) //Something forced us to cum, this is not a masturbation thing and does not progress to the other checks
		for(var/obj/item/organ/genital/G in internal_organs)
			...
			message_admins("[ADMIN_LOOKUPFLW(src)] came alone[G ? " with [p_their()] [G.name]" : ""].")
