...
/mob/living/carbon/human/build_virtual_character(mob/M, datum/outfit/outfit)
	...
	socks_color = H ? H.socks_color : sanitize_hexcolor(random_color())
	undershirt = H ? H.undershirt : random_undershirt(M.gender)
	shirt_color = H ? H.shirt_color : sanitize_hexcolor(random_color())
	underwear = H ? H.underwear : random_underwear(M.gender)
	undie_color = H ? H.undie_color : sanitize_hexcolor(random_color())
	give_genitals(TRUE)
	if(outfit)
