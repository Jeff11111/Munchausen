				...
				for(var/obj/machinery/airalarm/alarm in range(10, src)))
					if(alarm.powered() && alarm.radio)
						...
						if(istype(human_user))
							alarm.radio.talk_into(alarm, "Relief valve open pressure set to [open_pressure] by [human_user.get_id_name()] at [get_area_name(alarm, get_base_area = TRUE)]", alarm.radio_channel)
						else
							if(isliving(usr))
								alarm.radio.talk_into(alarm, "Relief valve open pressure set to [open_pressure] at [get_area_name(alarm, get_base_area = TRUE)]", alarm.radio_channel)
			...
			if(.)
				...
				for(var/obj/machinery/airalarm/alarm in range(10, src)))
					if(alarm.powered() && alarm.radio)
						var/mob/living/carbon/human/human_user = usr
						if(istype(human_user))
							alarm.radio.talk_into(alarm, "Relief valve close pressure set to [close_pressure] by [human_user.get_id_name()] at [get_area_name(alarm, get_base_area = TRUE)]", alarm.radio_channel)
						else
							if(isliving(usr))
								alarm.radio.talk_into(alarm, "Relief valve close pressure set to [close_pressure] at [get_area_name(alarm, get_base_area = TRUE)]", alarm.radio_channel)
