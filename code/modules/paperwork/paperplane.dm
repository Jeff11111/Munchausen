...
/obj/item/paperplane/suicide_act(mob/living/user)
	...
	LE?.receive_damage(brute=rand(8,12))
	RE?.receive_damage(brute=rand(8,12))
	sleep(10)
	return (BRUTELOSS)
...
/obj/item/paperplane/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(prob(hit_probability))
		...
		visible_message("<span class='danger'>\The [src] hits [H] in \the [chosen_eye]!</span>")
		H.adjust_blurriness(6)
		if(chosen_eye)
			chosen_eye.receive_damage(brute=rand(5,8))
