...
/datum/game_mode
	...
	var/list/required_jobs = list("Captain"=1, "Chief Enforcer"=1, "Logistics Officer"=1)		// alternative required job groups eg list(list(cap=1),list(hos=1,sec=2)) translates to one captain OR one hos and two secmans
...
/datum/game_mode/proc/convert_roundtype()
	...
	if(CONFIG_GET(flag/protect_assistant_from_antagonist))
		replacementmode.restricted_jobs += "Stowaway"
...
/datum/game_mode/proc/generate_credit_text()
	...
	for(var/datum/mind/current in SSticker.mode.get_all_by_department(GLOB.civilian_positions))
		if(current.assigned_role == "Stowaway")
