...
/obj/item/radio/talk_into(atom/movable/M, message, channel, list/spans, datum/language/language, direct = TRUE)
	if(!spans)
		spans = list(M.speech_span)
	if(!language)
		language = M.get_selected_language()
	SEND_SIGNAL(M, COMSIG_MOVABLE_RADIO_TALK_INTO, src, message, channel, spans, language, direct)
...
/obj/item/radio/borg
	...
	dog_fashion = null
	canhear_range = 1 //Giving away private channels is bad, nerfed range for borgs
