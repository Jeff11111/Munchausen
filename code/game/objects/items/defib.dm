			...
			if(H.stat == DEAD)
				...
				if(H.suiciding || HAS_TRAIT(H, TRAIT_NODEFIB) || (HAS_TRAIT(H, TRAIT_DNR)))
					...
					if(BR) //BUG_PROBABLE_CAUSE - slight difference between us and tg
						if(CHECK_BITFIELD(BR.organ_flags, ORGAN_DEAD))
				...
				if(failed)
					...
					H.revive()
					var/obj/item/organ/heart/they_heart = H.getorganslot(ORGAN_SLOT_HEART)
					if(they_heart)
						they_heart.artificial_pump(forced_pump = 0.6)
					H.emote("gasp")
					H.Jitter(100)
				...
				if(heart.Restart() && !heart.is_broken())
