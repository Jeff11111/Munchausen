...
/datum/component/boombox/proc/options(datum/source, mob/user)
	var/option = input(user, "What do i want to do with [parent]'s cassette module?", "Cassette Options", null) as null|anything in list("Toggle Loudness", "Change Range", "Change Volume", "Take Cassette Out")
	switch(option)
		if("Toggle Loudness")
			loud = !loud
...
/datum/component/boombox/proc/examine_message(datum/source, mob/user, list/examine_list)
	examine_list += "<span class='info'>\The [parent] can play cassette tapes.</span>"
