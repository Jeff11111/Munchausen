...
/datum/component/field_of_vision/proc/generate_fov_holder(mob/M, _angle = 0, _shadow_angle = 0, register = TRUE, delete_holder = FALSE)
	...
	if((M.client?.view && (M.client.view != "[current_fov_size[1]]x[current_fov_size[2]]")) || delete_holder)
...
#define FOV_ANGLE_CHECK(mob, target, zero_x_y_statement, success_statement) \
	...
	if(findtext(real_shadow_angle, "_"))\
		real_shadow_angle = copytext(real_shadow_angle, findtext(real_shadow_angle, "_"))\
