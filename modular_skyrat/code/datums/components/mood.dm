...
/datum/component/mood/process() //Called on SSobj process
	...
	switch(mood_level)
		...
		if(2)
			modify_sanity(-0.3, minimum=SANITY_LOST)
		if(3)
			modify_sanity(-0.2, minimum=SANITY_INSANE)	
		if(4)
			modify_sanity(-0.1, minimum=SANITY_CRAZY)
		if(5)
		...
		if(8)
			modify_sanity(+0.25, maximum=SANITY_GREAT)
		if(9)
