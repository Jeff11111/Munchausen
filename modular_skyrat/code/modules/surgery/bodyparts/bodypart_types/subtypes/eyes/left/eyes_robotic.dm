/obj/item/bodypart/left_eye/robot
	name = "robotic eyes"
	icon_state = "eye-c"
	desc = "Your vision is augmented."
	status = BODYPART_ROBOTIC | BODYPART_SYNTHETIC

/obj/item/bodypart/left_eye/robot/emp_act(severity)
	. = ..()
...
/obj/item/bodypart/left_eye/robot/xray
...
/obj/item/bodypart/left_eye/robot/thermals
	name = "thermal eyes"
...
/obj/item/bodypart/left_eye/robot/flashlight
...
/obj/item/bodypart/left_eye/robot/flashlight/emp_act(severity)
	return

/obj/item/bodypart/left_eye/robot/flashlight/attach_limb(mob/living/carbon/C, special, ignore_parent_restriction)
	. = ..()
	if(!eye)
...
/obj/item/bodypart/left_eye/robot/flashlight/drop_limb(special, ignore_children, dismembered, destroyed, wounding_type)
	. = ..()
	if(. && !QDELETED(owner))
		eye.on = FALSE
...
/obj/item/bodypart/left_eye/robot/shield
	name = "shielded robotic eyes"
	desc = "These reactive micro-shields will protect you from welders and flashes without obscuring your vision."
	flash_protect = 2

/obj/item/bodypart/left_eye/robot/shield/emp_act(severity)
	return
...
/obj/item/bodypart/left_eye/robot/glow
...
/obj/item/bodypart/left_eye/robot/glow/Initialize()
	. = ..()
	mob_overlay = image('icons/mob/human_face.dmi', "eyeglow_gs")

/obj/item/bodypart/left_eye/robot/glow/Destroy()
	terminate_effects()
	. = ..()

/obj/item/bodypart/left_eye/robot/glow/drop_limb(special, ignore_children, dismembered, destroyed, wounding_type)
	terminate_effects()
	. = ..()

/obj/item/bodypart/left_eye/robot/glow/proc/terminate_effects()
	if(owner && active)
		deactivate(TRUE)
	active = FALSE
	clear_visuals(TRUE)

/obj/item/bodypart/left_eye/robot/glow/ui_action_click(owner, action)
	if(istype(action, /datum/action/item_action/organ_action/toggle))
		toggle_active()
	else if(istype(action, /datum/action/item_action/organ_action/use))
		prompt_for_controls(owner)

/obj/item/bodypart/left_eye/robot/glow/proc/toggle_active()
	if(active)
		deactivate()
	else
		activate()

/obj/item/bodypart/left_eye/robot/glow/proc/prompt_for_controls(mob/user)
...
/obj/item/bodypart/left_eye/robot/glow/proc/assume_rgb(newcolor)
...
/obj/item/bodypart/left_eye/robot/glow/proc/cycle_mob_overlay()
	remove_mob_overlay()
	mob_overlay.color = current_color_string
	add_mob_overlay()

/obj/item/bodypart/left_eye/robot/glow/proc/add_mob_overlay()
	if(!QDELETED(owner))
		owner.add_overlay(mob_overlay)

/obj/item/bodypart/left_eye/robot/glow/proc/remove_mob_overlay()
	if(!QDELETED(owner))
		owner.cut_overlay(mob_overlay)

/obj/item/bodypart/left_eye/robot/glow/emp_act()
	. = ..()
	if(!active || . & EMP_PROTECT_SELF)
		return
	deactivate(silent = TRUE)

/obj/item/bodypart/left_eye/robot/glow/proc/activate(silent = FALSE)
	start_visuals()
	if(!silent)
...
/obj/item/bodypart/left_eye/robot/glow/proc/deactivate(silent = FALSE)
	clear_visuals()
	if(!silent)
...
/obj/item/bodypart/left_eye/robot/glow/proc/update_visuals(datum/source, olddir, newdir)
...
/obj/item/bodypart/left_eye/robot/glow/proc/clear_visuals(delete_everything = FALSE)
...
/obj/item/bodypart/left_eye/robot/glow/proc/start_visuals()
...
/obj/item/bodypart/left_eye/robot/glow/proc/set_distance(dist)
	light_beam_distance = dist
	regenerate_light_effects()

/obj/item/bodypart/left_eye/robot/glow/proc/regenerate_light_effects()
	clear_visuals(TRUE)
	on_mob = new(src)
	for(var/i in 1 to light_beam_distance)
...
/obj/item/bodypart/left_eye/robot/glow/proc/sync_light_effects()
	for(var/I in eye_lighting)
		var/obj/effect/abstract/eye_lighting/L = I
...
/obj/item/bodypart/left_eye/robot/glow/attach_limb(mob/living/carbon/C, special, ignore_parent_restriction)
	. = ..()
	if(.)
