...
/obj/item/bodypart/proc/decay()
	if(!can_decay())
		...
		return
	is_cold()
	if(CHECK_BITFIELD(status, BODYPART_FROZEN | BODYPART_DEAD))
		return
...
/obj/item/bodypart/proc/on_life()
	...
	if(stam_heal_tick && stamina_dam > DAMAGE_PRECISION)
		...
		var/pain_multiplier = max(0.1, 1 - (max(0, (get_pain() - owner?.chem_effects[CE_PAINKILLER])/max_pain_damage * (owner?.mind ? owner.mind.mob_stats[STAT_DATUM(end)].get_shock_mult() : 1))))
		if(!can_feel_pain())
			pain_multiplier = 1
...
/obj/item/bodypart/proc/update_limb(dropping_limb, mob/living/carbon/source)
	...
	if(!animal_origin)
		...
		colorlist.Cut()
		colorlist += "[H.dna.features["mcolor"]]"
		colorlist += "[H.dna.features["mcolor2"]]"
		colorlist +="[H.dna.features["mcolor3"]]"
